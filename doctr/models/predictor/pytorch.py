...
class OCRPredictor(nn.Module, _OCRPredictor):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        word_preds, images, masks = self.reco_predictor([crop for page_crops in crops for crop in page_crops], **kwargs)

        self.explanations = []
        for i in range(len(images)):
            self.explanations.append([word_preds[i][0], word_preds[i][1], images[i], masks[i]])
